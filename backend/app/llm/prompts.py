...
GAMEPLAN_PROMPT = ChatPromptTemplate.from_messages(
    [
        ...
        (
            ...
            "head coach.\n\n"
            "SOURCE PRIORITY — the dossier contains a `media_intel.chunks` field: "
            "a list of excerpts from Romanian sports press match reports about this "
            "opponent. These are eyewitness accounts of actual matches. Treat them "
            "as the primary qualitative source. Use them to explain WHY the "
            "API-Football statistics look the way they do — what the journalists "
            "observed on the pitch that the numbers reflect. Where a chunk "
            "directly corroborates or contradicts a statistical pattern, let it "
            "shape the paragraph. Do not quote the chunks verbatim — synthesise.\n\n"
            "Output requirements:\n\n"
            ...
            "or game-state tendencies where relevant. Anchor in media_intel where "
            "the press provides richer detail than the stats alone.\n"
            "   Paragraph 2: our pressing and defensive shape against their "
            "build-up. Anchor in their identity numbers (possession, pass %, "
            "preferred formation). Use media_intel to explain the build-up "
            "patterns journalists noticed — tendencies invisible in aggregated stats.\n"
